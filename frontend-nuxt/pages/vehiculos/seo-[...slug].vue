<template>
  <div class="min-h-screen bg-slate-950 text-white">
    <!-- Redirigir al componente de detalle de vehículo -->
    ...
    <div v-else-if="vehiculo" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Incluir el mismo contenido que [id].vue -->
      <!-- Galería de imágenes -->
...
// Extraer ID del slug SEO-friendly
const extractIdFromSlug = (slug) => {
  // El slug tiene formato: auto-usado-[marca]-[modelo]-[año]-[ciudad]-[id]
  // Extraemos el último segmento que es el ID
...
const cargarVehiculo = async () => {
  try {
    ...
    if (Array.isArray(slug)) {
      // Si es un array, unir con guiones y extraer ID
      vehiculoId = extractIdFromSlug(slug.join('-'))
    } else {
      vehiculoId = extractIdFromSlug(slug)
    }
