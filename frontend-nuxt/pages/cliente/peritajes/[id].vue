<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    {{ console.log('🟢 [Peritaje Detail] Template renderizando, ID:', route.params.id) }}
...
const cargarPeritaje = async () => {
  try {
    console.log('🟢 [Peritaje Detail] Iniciando carga, ID:', route.params.id)
    loading.value = true
    const peritajeStore = usePeritajeStore()
    peritaje.value = await peritajeStore.fetchPeritajeById(route.params.id)
    console.log('🟢 [Peritaje Detail] Peritaje cargado:', peritaje.value)
  } catch (error) {
