<template>
    <div class="min-h-screen bg-gray-100 text-gray-800 p-4 md:p-8">
      ...
      <div v-if="!esDetalle">
        ...
        <div v-if="vehiculos.length === 0" class="text-gray-500">No se encontraron resultados.</div>
  ...
  const esDetalle = computed(() => id && !isNaN(id))
  
  // 🧠 Ruta base SEO-friendly
  const ruta = `/vehiculos${route.fullPath}`
  
  onMounted(async () => {
    try {
      const res = await api.get(`/vehiculos${route.fullPath}`)
      if (esDetalle.value) {
  ...
  function generarRutaDetalle(v) {
    ...
    return `/${slug(v.region?.nombre)}/${slug(v.comuna?.nombre)}/${slug(v.marca?.nombre)}/${slug(v.modelo?.nombre)}/${v.anio}/${v.id}`
  }
  </script>
